...
cmd:option('-config', 'config', [[Read options from this file]])

cmd:text("")
cmd:text("**Data options**")
cmd:text("")

cmd:option('-src', 'input.txt', [[Source sequence to decode (one line per sequence)]])
cmd:option('-tgt', '', [[True target sequence (optional)]])
cmd:option('-output', 'output.txt', [[Path to output the predictions (each line will be the decoded sequence]])
...
cmd:text("")
cmd:option('-gpuid', -1, [[1-based identifier of the GPU to use. CPU is used when the option is < 1]])
